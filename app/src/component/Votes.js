...
export default class Votosnegativos extends React.Component {
    ...
    verify(type) {
        if (this.props.userLoggin !== 0) {
            if (type === 'positive') {
                let esta = this.state.usersUpvotes.filter((vote) => vote === this.props.userLoggin);
                let arregloDownvotes = this.state.usersDownvotes.filter((vote) => vote !== this.props.userLoggin);
                if (esta.length === 1) {
                    Swal.fire({
                        position: 'center',
                        icon: 'Information',
                    ...
                    this.props.addVotos(this.state.usersUpvotes, this.props.memeId ,arregloDownvotes);
                }
            } else {
                let esta = this.state.usersDownvotes.filter((vote) => vote === this.props.userLoggin);
                let arregloUpvotes = this.state.usersUpvotes.filter((vote) => vote !== this.props.userLoggin);
                if (esta.length === 1) {
                    Swal.fire({
                        position: 'center',
                        icon: 'Information',
                    ...
                    this.setState({
                        countUpVotes: this.state.countUpVotes - 1,
                        countDownVotes: this.state.countDownVotes + 1,
                        usersDownvotes: this.state.usersDownvotes.push(this.props.userLoggin)
                    });
                    this.props.addVotos(arregloUpvotes, this.props.memeId, this.state.usersDownvotes);
                }
            }
        } else {
            Swal.fire({
